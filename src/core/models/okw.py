...
class ManufacturingFacility:
    ...
    def from_dict(cls, data: Dict) -> "ManufacturingFacility":
        ...
        what3words = (
            What3Words(
                words=w3w_data.get("words", ""), language=w3w_data.get("language", "")
            )
            if w3w_data and "words" in w3w_data
            else None
        )
        ...
        facility = cls(name=name, location=location, facility_status=facility_status)
        ...
        facility.uninterrupted_power_supply = data.get(
        ...
        if hc_data:
            facility.human_capacity = HumanCapacity(headcount=hc_data.get("headcount"))
